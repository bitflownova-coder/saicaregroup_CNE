...
registrationSchema.index({ formNumber: 1 });
...
registrationSchema.statics.getNextFormNumber = async function() {
  const last = await this.findOne({}, { formNumber: 1 }).sort({ formNumber: -1 }).lean();
  return (last?.formNumber || 0) + 1;
};

// Static method to get registration count
registrationSchema.statics.getRegistrationCount = async function() {
  return await this.countDocuments();
};

// Static method to check if registration is full
registrationSchema.statics.isRegistrationFull = async function() {
